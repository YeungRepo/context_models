...
function[dx] = TandModelJournal(t,x,x0,dms,dmg,Rtot,pLactot,kr,pTettot,Omega,h0,TLS,TLG,NS,PLS,...
...
DeltaKink = (spS+stG)*h0;


nfS = PLS + pTet/(pTetC+pTet)*NS/2 + pTetC/(pTetC+pTet)*(TLG+NS/2) - DeltaKink;
nfG = TLG + NS/2 +DeltaKink;
...
% Pulled out of dx(5) backbone barrier dynamics b/c backbone data in
%Supplement Figure 3-4(sense vs antisense single gene_
%suggests free propagation of supercoils is allowed via backbone genes. 
dx(5) = Omega*(1/(2*h0)*((dECS-dCCS)*PLS/nfS) ... 
     + m_gyrtopfunc(stS,pLactot,s0,T0,G0,tau,gamma,TLS,pLen, kMgyr)); 

%sigmatG
dx(6) = Omega*(1/(2*h0)*((dMGcr-dECG)*TLG/nfG + (dECG-dCCG)*PLG/PLG )...
...
%sigmapG
dx(8) = Omega*(1/(2*h0)*( (dECG-dCCG)*PLG/PLG  )  ...
